...
#include "Lgm/praxis.h"

void praxis(int n, double *x, int *data, double (*funct)(double *, void *data),
            double *in, double *out) {

  int illc, i, j, k, k2, nl, maxf, nf, kl, kt, ktm, emergency;
  double s, sl, dn, dmin, fx, f1, lds, ldt, sf, df, qf1, qd0, qd1, qa, qb, qc,
      m2, m4, small, vsmall, large, vlarge, scbd, ldfac, t2, macheps, reltol,
  ...
  while (1) {
    sf = d[1];
    d[1] = s = 0.0;
    praxismin(1, 2, &(d[1]), &s, &fx, 0, n, x, v, &qa, &qb, &qc, qd0, qd1, q0,
              q1, &nf, &nl, &fx, m2, m4, dmin, ldt, reltol, abstol, small, h,
              funct, data);
    if (s <= 0.0)
      mulcol(1, n, 1, 1, v, v, -1.0);
    ...
    for (k = 2; k <= n; k++) {
      ...
      while (1) {
        ...
        for (k2 = k; k2 <= n; k2++) {
          sl = fx;
          s = 0.0;
          praxismin(k2, 2, &(d[k2]), &s, &fx, 0, n, x, v, &qa, &qb, &qc, qd0,
                    qd1, q0, q1, &nf, &nl, &fx, m2, m4, dmin, ldt, reltol,
                    abstol, small, h, funct, data);
      ...
      for (k2 = 1; k2 <= k - 1; k2++) {
        s = 0.0;
        praxismin(k2, 2, &(d[k2]), &s, &fx, 0, n, x, v, &qa, &qb, &qc, qd0, qd1,
                  q0, q1, &nf, &nl, &fx, m2, m4, dmin, ldt, reltol, abstol,
                  small, h, funct, data);
      }
      f1 = fx;
      fx = sf;
      ...
      if (lds > small) {
        ...
        praxismin(k, 4, &(d[k]), &lds, &f1, 1, n, x, v, &qa, &qb, &qc, qd0, qd1,
                  q0, q1, &nf, &nl, &fx, m2, m4, dmin, ldt, reltol, abstol,
                  small, h, funct, data);
        if (lds <= 0.0) {
          lds = -lds;
    ...
    if ((qd0 * qd1 > DBL_MIN) && (nl >= 3 * n * n)) {
      praxismin(0, 2, &s, &l, &qf1, 1, n, x, v, &qa, &qb, &qc, qd0, qd1, q0, q1,
                &nf, &nl, &fx, m2, m4, dmin, ldt, reltol, abstol, small, h,
                funct, data);
    ...
    for (i = 1; i <= n; i++) {
      s = dn * d[i];
      d[i] = (s > large) ? vsmall : ((s < small) ? vlarge : 1.0 / (s * s));
    }
    /* sort */
    for (i = 1; i <= n - 1; i++) {
      k = i;
...
int qrisngvaldec(double **a, int m, int n, double *val, double **v,
                 double *em) {
  double *b = allocate_real_vector(1, n);
  hshreabid(a, m, n, val, b, em);
  psttfmmat(a, n, v, b);
  pretfmmat(a, m, n, val);
  int i = qrisngvaldecbid(val, b, m, n, a, v, em);
...
double **allocate_real_matrix(int lr, int ur, int lc, int uc) {
  double **p = (double **)malloc((unsigned)(ur - lr + 1) * sizeof(double *));
  if (!p) {
